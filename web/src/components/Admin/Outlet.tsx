...
interface AdminPageOutletProps {
  addName: string;
  fetchUrl: string;
  dataRender: (data: any) => { title: string; forDelete: any }[];
  deleteElement: (data?: string) => void;
  searchName: string;
}

export function AdminPageOutlet(props: AdminPageOutletProps) {
  const { addName, fetchUrl, dataRender } = props;
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        setLoading(1);
        const data = await fetch(`${MAIN_URL}${fetchUrl}/?search=${search}`);
            ...
            {dataRender(data).map((element, index) => {
              return (
                ...
                >
                  <p>{element.title}</p>
                  <img
                    onClick={() => props.deleteElement(element.forDelete)}
                    src={cross}
                    width={15}
                    height={15}
